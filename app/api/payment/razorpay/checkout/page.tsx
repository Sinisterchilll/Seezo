    ...
    try {
      const options = {
        ...
        handler: async function (response: any) {
          ...
          const result = await fetch("/api/payment/razorpay/verify", {
            method: "POST",
            body: JSON.stringify(verifyData),
            headers: { "Content-Type": "application/json" },
          });
          const res = await result.json();
          console.log("Verification response:", res); // Log the full response for debugging

          if (res.isOk) {
            alert(res.message || "Payment successful! Your subscription has been updated.");
            router.push("/"); // Redirect to dashboard or confirmation page
          } else {
            console.error("Payment verification failed:", res.message || "Unknown error");
            alert(`Payment verification failed: ${res.message || "Unknown error"}. Please contact support.`);
          }
        },
        prefill: {
          name: "Customer Name",
